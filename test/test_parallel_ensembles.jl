...
function test_parallel_multicanonical(; verbose=false)
    ...
    ensemble(muca).histogram.weights .= [1.0, 2.0, 3.0, 4.0]
    ...
    @test all(ensemble(muca).histogram.weights .== [1.0, 2.0, 3.0, 4.0])

    # Test logweight distribution (broadcast)
    ensemble(muca).logweight.weights .= [10.0, 20.0, 30.0, 40.0]
    distribute_logweight!(pmuca, ensemble(muca).logweight)
    # Should remain unchanged in COMM_SELF
    @test all(ensemble(muca).logweight.weights .== [10.0, 20.0, 30.0, 40.0])
