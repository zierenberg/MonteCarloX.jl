...
@testset "structure_factor matches Fourier reference" begin
    ...
    for (make, dims) in ((d -> NonreciprocalIsing(d; κ=0.3), [4, 6]),
                         (d -> NonreciprocalIsing(d; κ=0.3), [3, 4, 5]),
                         (d -> NonreciprocalBlumeCapel(d; κ=0.3, D=0.5), [6, 4]))
        sys = make(dims)
        init!(sys, :random, rng=rng)
        for d in eachindex(dims)
            ref = reference_structure_factor(sys.spins, sys.topo.dims, d)
...
@testset "structure_factor of a single localized spin is 1" begin
    ...
    sys = NonreciprocalBlumeCapel([4, 6]; κ=0.0, D=0.0)
    for site in (1, 7, 24)
        init!(sys, :zero)
...
@testset "structure_factor vanishes for uniform configurations" begin
    sys = NonreciprocalIsing([8, 8]; κ=0.5)
    for type in (:up, :down)
        init!(sys, type)
...
@testset "structure_factor symmetries" begin
    ...
    sys = NonreciprocalIsing([4, 6]; κ=0.3)
    init!(sys, :random, rng=rng)
