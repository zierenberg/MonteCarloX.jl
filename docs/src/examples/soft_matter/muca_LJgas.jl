...
# set initial weights from previous run to speed up convergence
set!(ensemble(alg), logweight_ref, rescale_bins = N/N_ref)
# run a few iterations to verify that the algorithm can thermalize the larger system and record round trips
...
# ## Plots for larger system
plot_histograms_and_logweights("E/N", histograms, logweights; title_prefix="LJ gas MUCA (N=$(N)) naive init")