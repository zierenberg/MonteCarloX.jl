# Renders this folder's SmoQyDQMC interop examples into markdown, with `execute = true` so
# Literate runs each script itself and bakes the output in as static text — no SmoQyDQMC
...
#
# Run: `julia --project=examples/external/smoqy examples/external/smoqy/build_docs.jl [--rerun]`
